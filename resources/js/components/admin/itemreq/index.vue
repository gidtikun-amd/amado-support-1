<template>
    <div>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <ul class="nav nav-tabs" id="" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="processing-tab" data-toggle="tab" href="javascript:;" role="tab" aria-controls="processing" aria-selected="false"
                                    v-on:click="getListData('inprogress')">
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="col-auto">
                                            ...
                                            v-on:keyup.enter="getDataPaginate('first')">
                                                ...
                                                <option value="0">ยังไม่ได้อัพโหลดเอกสาร</option>
                                            </select>
                                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body mx-4 mt-5">
                        ...
                        <div class="row justify-content-center py-3"
                            v-show="detail.data.header.itmreq_status == 1 || detail.data.header.itmreq_status == 2">
                            ...
                            <button type="button" class="col-md-3 btn btn-warning mr-3"
                                v-show="detail.data.header.itmreq_status == 1"
                                v-on:click="updateItmreqStatus(2)">
        ...
        <div class="modal fade" id="modal-remark-void" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <i class="fas fa-sticky-note text-primary" > ใส่เหตุผลการยกเลิก</i>
                    </div>
                    <div class="modal-body text-center" >
                        <div class="row">
                            <div class="col-12">
                                <textarea class="form-control" rows="4" v-model="detail.voidRemark"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" v-on:click="setVoidItmreq('save')">ทำการยกเลิกรายการ</button>
                        <button type="button" class="btn btn-danger" v-on:click="setVoidItmreq('cancel')">ออก</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods:{
        ...
        getListData(status){
            ...
            axios.post('/api/admin/it/itemrequest/list',{
        ...
        updateItmreqStatus(status){
            ...
            if( status == 0 ){
                var title_txt = 'ยืนยันการยกเลิก',
                    content_txt = 'ต้องการยกเลิก '+this.data[this.detail.index].itmreq_no+' หรือไม่';
            }
            else{
                var title_txt = 'ยืนยันการเปลี่ยนสถานะ',
                    content_txt = 'ต้องการเปลี่ยนสถานะ '+this.data[this.detail.index].itmreq_no+' หรือไม่';
            }
            ...
            .then((result) => {
                if( result.isConfirmed ){
                    if(status != 0){
                        ...
                        $('#modal-itemreq').modal('hide');
                        $('#modal-remark-void').modal('show');
                    }
                }
            });

        },
        setVoidItmreq(action){

            if(this.detail.voidRemark == '' && action == 'save'){
                this.$swal({icon: 'error', text: 'กรุณาใส่เหตุผลการยกเลิก'});
                return;
            }
            ...
            if(action == 'save'){
                axios.post('/api/admin/it/itemrequest/udstatus',{
                    id: this.detail.itmreq_id,
                    status: 0,
                    remark: this.detail.voidRemark,
                    user: this.$session.get('user')
                })
                .then(response => {
                    if(response.data.status){
                        this.$swal({ icon: 'success', text: 'เปลี่ยนแปลงรายการเรียบร้อยแล้ว' });
                        $('#modal-remark-void').modal('hide');
                        // this.getItmreqDetail(this.detail.index);
                        this.getListData(this.paginate.method);
                    }
                    else{
