                    ...
                    <div class="modal-body mx-4 mt-5">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-12">
                                                <b>Approve :</b>
                                                <span v-show="detail.header.itmreq_approve == 0">
                                                    <i class="fas fa-exclamation-circle text-warning"></i> 
                                                    <b>ยังไม่ได้ Approve</b>
                                                </span>
                                                <span v-show="detail.header.itmreq_approve == 1 && detail.header.itmreq_status == 3">
                                                    <i class="fas fa-check-circle text-success"></i> 
                                                    <b>Approve By {{detail.approve.it_emp_fullname}} ({{detail.approve.user}})</b>
                                                </span>
                                                <span v-show="detail.header.itmreq_approve == 2">
                        ...
                        <div class="row mt-3">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <i class="fas fa-folder-open"></i> <b>API Log</b>
                                    </div>
                                    <div class="card-body">   
                                        <button type="button" class="btn btn-primary btn-sm"
                                        v-on:click="resendApi(detail.header.itmreq_id,detail.header.itmreq_no)">
                                            Resend API <i class="fas fa-arrow-alt-circle-right ml-1"></i> 
                                        </button>                                 
                                    </div>
                                    <table class="table table-hover table-bordered table-sm">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-center" width="15%">ref number</th>
                                                <!-- <th class="text-center">json</th> -->
                                                <th class="text-center" width="20%">status</th>
                                                <th class="text-center" width="">Api messsage</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr v-for="(a,index) in detail.api" :key="index">
                                                <td class="text-center">{{index+1}}</td>
                                                <td>{{a.created_at}}</td>
                                                <td>{{a.api_ref}}</td>
                                                <!-- <td>{{a.api_json_send}}</td> -->
                                                <td>
                                                    <span class="text-primary" v-if="a.api_status == 1">
                                                        <i class="fas fa-pause-circle mr-1"></i>
                                                        <b>ส่งแล้วรอ CRM ตอบกลับ</b>
                                                    </span>
                                                    <span class="text-danger" v-else-if="a.api_status == 2">
                                                        <i class="fas fa-exclamation-triangle mr-1"></i>
                                                        <b>Error</b>
                                                    </span>
                                                    <span class="text-success" v-else-if="a.api_status == 3">
                                            ...
                                            <tr v-show="detail.api.length <= 0">
                                                <td colspan="5" class="text-center"><b>ไม่มีรายการ</b></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
    ...
    methods:{
        ...
        showDetailItmReq(index){
            ...
            .then(response => {
                ...
                this.detail.api = response.data.api;
                this.detail.approve = response.data.approve;
        ...
        resendApi(id,no){
            this.$swal({
                icon: 'warning',
                title: 'Resent API',
                text: 'ต้องการ Resend API '+no+' หรือไม่',
                showDenyButton: true,
                showCancelButton: false,
                confirmButtonText: 'ยืนยัน',
                denyButtonText: 'ยกเลิก'
            ...
            .then((result) => {
                if(result.isConfirmed){
                    axios.get('/api/crm/item-request/resend-api/'+id)
                    .then(response => {
                        if(response.data.Status){
                            this.$swal({ 
                                icon: 'success', 
                                text: 'Resend API '+no+' เรียบร้อยแล้ว' 
                            });
